...
RF <- cv(bacteria, 'Genus', randomForest, ntree = 1000)
SVM <- cv(bacteria, 'Genus', probability = TRUE, svm, kernel = 'radial')
LR <- cv(bacteria, 'Genus', glm, family = binomial)
b_all_genus_method <- rbind(RF, SVM, LR)
b_all_genus_method$true <- as.factor(b_all_genus_method$true)
# 绘制ROC曲线
g1 <- b_all_genus_method %>% 
  group_by(method) %>% 
  roc_curve(true, prob) %>%
  autoplot() + theme_test() + 
